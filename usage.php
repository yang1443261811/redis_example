 ...
 */
//从list头部插入一个值。
//$ret = $redis->lPush('hometown', 'nanjing');

//从list尾部插入一个值。
//$ret = $redis->rPush('hometown', 'guangzhou');
//$ret = $redis->rPush('hometown', 'guiyang');
//$ret = $redis->rPush('hometown', 'shanghai');
//$ret = $redis->rPush('hometown', 'changsha');
//$ret = $redis->rPush('hometown', 'beijing');

//获取列表指定区间中的元素。0表示列表第一个元素，-1表示最后一个元素，-2表示倒数第二个元素。
//$ret = $redis->lrange('hometown', 0, -1);//查看队列所有元素
//print_r($ret);

//将一个值插入已存在的列表头部，列表不存在时操作无效。
//$ret = $redis->lPushx('hometown', 'suzhou');

//将一个或多个值插入已存在的列表尾部，列表不存在时操作无效。
//$ret = $redis->rPushx('hometown', 'zhengzhou');

//移除并返回列表的第一个元素，若key不存在或不是列表则返回false。
//$ret = $redis->lPop('hometown');

//移除并返回列表的最后一个元素，若key不存在或不是列表则返回false。
//$ret = $redis->rPop('hometown');

//移除并获取列表的第一个元素。如果列表没有元素则会阻塞列表直到等待超时或发现可弹出元素为止。
//参数：key，超时时间（单位：秒）
//返回值：[0=>key,1=>value]，超时返回[]
//$ret = $redis->blPop('hometown', 10);

//移除并获取列表的最后一个元素。如果列表没有元素则会阻塞列表直到等待超时或发现可弹出元素为止。
//参数：key，超时时间（单位：秒）
//返回值：[0=>key,1=>value]，超时返回[]
//$ret = $redis->brPop('hometown', 10);

//移除列表中最后一个元素，将其插入另一个列表头部，并返回这个元素。若源列表没有元素则返回false。
//$ret = $redis->rpoplpush('hometown', 'city2');

//移除列表中最后一个元素，将其插入另一个列表头部，并返回这个元素。如果列表没有元素则会阻塞列表直到等待超时或发现可弹出元素为止。
//参数：源列表，目标列表，超时时间（单位：秒）
//超时返回false
//$ret = $redis->brpoplpush('hometown', 'city2', 10);

//返回列表长度。
//$ret = $redis->lLen('hometown');

//通过索引获取列表中的元素。若索引超出列表范围则返回false。
//$ret = $redis->lindex('hometown', 0);

//通过索引设置列表中元素的值。若是索引超出范围，或对一个空列表进行lset操作，则返回false。
//$ret = $redis->lSet('hometown', 3, 'xian');
...
//$ret = $redis->lInsert('hometown', Redis::AFTER, 'beijing', 'nanjing');
...
//返回实际删除元素个数
//$ret = $redis->lrem('hometown', 'guangzhou', -2);

//对一个列表进行修剪，只保留指定区间的元素，其他元素都删除。成功返回true。
//$ret = $redis->ltrim('hometown', 1, 4);
...
//$ret = $redis->sAdd('myset', 'age');
//$ret = $redis->sAdd('myset', 'salary');
//$ret = $redis->sAdd('myset', 'wight');
//$ret = $redis->sAdd('myset', 'height');
//$ret = $redis->sAdd('myset', 'level');
//$ret = $redis->sAdd('myset', 'Education');
...
//$ret = $redis->zAdd('scores', 35, 'English', 95, 'physics', 75, 'math', 45, 'sports', 94, 'history', 98, 'music');
